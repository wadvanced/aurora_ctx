defmodule Aurora.Ctx.Test.Cases.OptionCreateChangesetReferenceTest do
  ...
  defmodule Inventory do
    ...
    ctx_register_schema(Product, create_changeset: &Inventory.Product.create_changeset/2)
  end
  ...
  test "Test create changeset function" do
    ...
    |> tap(&refute(&1.description == "An Item to be the first"))
    |> tap(&assert(&1.cost == Decimal.new("11.12")))
    |> context.update_product(%{
      description: "Item to be the first one",
      name: "New Item 001",
      list_price: 40.05
    })
    ...
    |> tap(&assert(&1.list_price == Decimal.new("40.05")))
  end
end
